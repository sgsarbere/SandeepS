...
namespace EngTron
{
    ...
    public class PositionP4S : Positions
    {
        ...
        public override void Perform(int i, int j)
        {
            ...
            {
                do { k1++; }
                while (cases[1][nbLi - 1 + k1 * nbLi]
                || cases[0][nbLi - 1 + k1 * nbLi]
                || cases[2][nbLi - 1 + k1 * nbLi]);
                {
                    h++;
                }
            }
            ...
            {
                do { k0++; }
                while (cases[1][nbLi - 1 + k0 * nbLi]
                || cases[0][nbLi - 1 + k0 * nbLi]
                || cases[2][nbLi - 1 + k0 * nbLi]);
                {
                    h++;
                }
            }
            ...
            if (re[0])
            {
                NbShots0 = 0; NbShots1 = 0; Eval = -1; return;
            }
